...
export default async (req) => {
  ...
  /* Anthropic_Bhanu_Chat is the variable Bhanu actually set (Netlify locks a
     secret variable's key after creation, so the code bends rather than the
     name). It is deliberately FIRST: the runtime also carries an
     ANTHROPIC_API_KEY that nobody put in the site's env list — injected by
     Netlify — and it answers 401 against api.anthropic.com. Preferring the
     conventional name here silently handed every request to that key. */
  const key = process.env.Anthropic_Bhanu_Chat || process.env.ANTHROPIC_API_KEY;
  if (!key) {
